...
export function formatWeeklyReportMarkdown(report: WeeklyReport, t: (key: string) => string, language: string): string {
  ...
  if (hours > 0) {
    if (language === 'en') {
      readingDisplay = `${hours}${hourUnit} ${mins}${minuteUnit}`;
    } else {
      readingDisplay = `${hours}${hourUnit}${mins}${minuteUnit}`;
    }
  } else {
    readingDisplay = `${mins}${minuteUnit}`;
  }

  let md = `# 📊 ${t('report.title')}\n\n`;
  md += `> ${t('report.statsPeriod')}：${fmt(start)} — ${fmt(end)}\n\n`;
  md += `---\n\n`;
  md += `## 📈 ${t('report.overview')}\n\n`;
  md += `| ${t('report.metric')} | ${t('report.value')} |\n`;
  md += `| --- | --- |\n`;
  md += `| ${t('report.reviewCompletion')} | ${report.reviewCompletionRate}% |\n`;
  md += `| ${t('report.newCards')} | ${report.newCardsCount}${t('report.newCardsUnit')} |\n`;
  md += `| ${t('report.newLinks')} | ${report.newLinksCount}${t('report.newLinksUnit')} |\n`;
  md += `| ${t('report.totalReading')} | ${readingDisplay} |\n`;
  md += `| ${t('report.activeDaysLabel')} | ${report.activeDays}${t('report.weekDaysUnit')} |\n`;
  md += `| ${t('report.currentStreak')} | ${report.currentStreak}${t('report.currentStreakUnit')} |\n`;
  md += `| ${t('report.reviewCount')} | ${report.totalReviews}${t('report.visitCountSuffix')} |\n\n`;

  md += `## 🏆 ${t('report.topVisited')}${report.topVisitedCards.length}\n\n`;
  if (report.topVisitedCards.length > 0) {
    md += `| ${t('report.rank')} | ${t('report.card')} | ${t('report.visitCount')} |\n`;
    md += `| --- | --- | --- |\n`;
    report.topVisitedCards.forEach((item, i) => {
      md += `| ${i + 1} | ${item.cardTitle} | ${item.visitCount}${t('report.visitCountSuffix')} |\n`;
    });
  } else {
    md += `${t('report.noRecords')}\n`;
  }
  md += `\n---\n\n`;
  md += `*${t('report.autoGenerated')}*\n`;
